...
export default function Bracket({ currentRound }: Props) {

    const { liveBracket, bracket } = useModalContext()

    switch (bracket.size) {
        case 8: return (
            <div className={`bracket__main-8 currentround-${currentRound}`}>
        ...
        default: return (
            <div className={`bracket__main-4 currentround-${currentRound}`}>
                <div className='bracket__round2'>
                    <Matchup
                        currentRound={currentRound}
                        matchup={bracket.rounds.final4.matchups[0]}
